...
class Proton:
    def __init__(self):
        """
        Partícula que representa um próton
        """
        self._carga = '+'
    ...
    def mover_para(self, x:float, y:float):
        """
        Função para movimentar o próton
        
        Args:
            x (float): Coordenada cartesiana no eixo X para movimentação do próton
            y (float): Coordenada cartesiana no eixo Y para movimentação do próton
        """
        self._desenho.up()
...
class Eletron:
    def __init__(self):
        """
        Definindo forma da representação do elétron
        """
        self._carga = '-'
        self._angulo = None
